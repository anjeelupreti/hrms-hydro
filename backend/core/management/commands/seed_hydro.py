...
class Command(BaseCommand):
    ...
    def _meetings(self, companies, people):
        """Meetings in every circumstance one can be in.

        **Moved out to `notifications.seeding`,** because the interesting part
        is no longer "a meeting exists" — it is scheduled, ended and cancelled,
        seen from both sides of the table, each with the agenda, register,
        decisions and minute at a different point. That is a demonstration in
        its own right and is run on its own (`manage.py seed_meetings`) against
        databases that already have people in them.
        """
        from notifications.seeding import seed_meetings

        seed_meetings(for_username="owner", stdout=self.stdout)
